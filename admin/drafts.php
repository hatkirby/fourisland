...
if (isset($_GET['action']))
{
	if ($_GET['action'] == 'delete')
	{
		...
		{
			$delpost = "DELETE FROM drafts WHERE id = " . $_POST['id'];
			$delpost2 = mysql_query($delpost);

			$flashmsg = 'The selected draft has been deleted.';
		}
	} else if ($_GET['action'] == 'deletes')
	{
		...
		if (is_array($ids) && !empty($ids))
		{
			foreach ($ids as $id)
			{
				$delpost = "DELETE FROM drafts WHERE id = " . $id;
				$delpost2 = mysql_query($delpost);
			}

			$flashmsg = 'The selected drafts have been deleted.';
		}
	}
}

$template = new FITemplate('admin/drafts');

$getposts = "SELECT * FROM drafts ORDER BY id ASC";
